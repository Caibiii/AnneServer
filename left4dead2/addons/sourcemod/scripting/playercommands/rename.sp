        ...
        if (IsBotInfected(i) && IsPlayerAlive(i))
        {
			...
			switch (type)
			{
				case 1:
				{
					...
					if(count >= FindConVar(cvar).IntValue)
					{
						ArraySpecial[0] = 0;
					}
				}
				case 2:
				{
					class2++;
					char cvar[16];
					Format(cvar, 16, "z_%s_limit", infected_name[class2]);
					if(count >= FindConVar(cvar).IntValue)
					{
						ArraySpecial[1] = 0;
					}
				...
				case 4:
				{
					class4++;
					char cvar[16];
					Format(cvar, 16, "z_%s_limit", infected_name[class4]);
					if(count >= FindConVar(cvar).IntValue)
					{
						ArraySpecial[3] = 0;
					}
