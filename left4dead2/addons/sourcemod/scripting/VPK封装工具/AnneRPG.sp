...
public Action:Command_Setinfo(client, const String:command[], args)
{
	...
	if(player_data[client][Intelligence] > 0  &&  GetClientTeam(client) == 3)
	{
		player_data[client][Intelligence] -= 1;
	}
    if (StrEqual(arg, "3", false))
        return Plugin_Handled;
...
for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 3 && player_data[i][Intelligence] < 1)
		{
...
public OnClientDisconnect(Client)
{
	...
	{
		Initialization(Client);
		if(player_data[Client][Intelligence] > 0)
		{
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (player_data[victim][Intelligence] > 0) 
	{
		player_data[victim][CASH] -= 200;
		player_data[attacker][CASH] += 200;
	}
...
/* 玩家受伤 */
/* 快捷指令 */
public Action:AddStrength(Client, args) //近战
{
...
public Action:AddEndurance(Client, args) //回血
{
...
public Action:AddIntelligence(Client, args) //内鬼
{
	new humans = GetInfectedCount();		
	if(player_data[Client][CASH] > 200 && humans == 0)
	{
		if (args < 1)
		{
			if(player_data[Client][Intelligence] + 1 > 1)
			{
				return Plugin_Handled;
			}
			else
			{
				...
				PrintToChat(Client,"\x05[提示]如果主动更换阵营自动视为放弃当内鬼!");
		...
		if (player_data[Client][CASH] >= StringToInt(arg))
		{
			if(player_data[Client][Intelligence] + StringToInt(arg) > 1)
			{
				return Plugin_Handled;
			}
			else
	...
	else if (humans != 0 && player_data[Client][Intelligence] == 0)
	{
		PrintToChat(Client,"\x05当前服务器中已存在一只内鬼,下关再试试？");
	}
	return Plugin_Handled;
}
public Action:ResetBshu(Client, args) //重置技能
{
	if(player_data[Client][CASH] > 0)
	{
		if (args < 1)
		{
			...
			player_data[Client][CASH] += (player_data[Client][Intelligence] * 200 +player_data[Client][Endurance] * 1000 +player_data[Client][Str] * 300);
			player_data[Client][Intelligence] = 0;
...
public Action:MenuFunc_AddStatus(Client)
{
	...
	DrawPanelText(menu, line);
	Format(line, sizeof(line), "无内鬼 (%d/1) 每级200点B数", player_data[Client][Intelligence]);
	DrawPanelItem(menu, line);
	Format(line, sizeof(line), "进入特感阵营 每次死亡减少200点B数");
	DrawPanelText(menu, line);
	Format(line, sizeof(line), "重置技能  免费(返还技能消耗的B数)");
	DrawPanelItem(menu, line);
		...
		switch(param)
			{
				...
				case 3:	AddIntelligence(Client, 0);
				case 4:	ResetBshu(Client, 0);
			}
