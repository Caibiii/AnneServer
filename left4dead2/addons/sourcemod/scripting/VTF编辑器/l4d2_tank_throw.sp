...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	if (StrEqual(wpname, "tank_claw", false))
	{
		//ServerCommand("sm_cvar z_tank_throw_force 400");
		SetConVarString(FindConVar("z_tank_throw_force"), "400");
		return Plugin_Changed;
	}
	return Plugin_Continue;
}
public Action:AnneHappy(Handle:timer)
{
	//ServerCommand("sm_cvar z_tank_throw_force 400");
	SetConVarString(FindConVar("z_tank_throw_force"), "400");
	return Plugin_Stop;
 ...
 */
#define TANK_MELEE_SCAN_DELAY 0.0
...
stock Action:onTankRunCmd(client, &buttons, Float:vel[3], Float:angles[3])
{
    if (buttons & IN_ATTACK2) 
	{
		...
		CreateTimer(3.0, AnneHappy, _, TIMER_FLAG_NO_MAPCHANGE);
...
stock any:nearestSurvivorDistance(client)
{
        ...
        for (new i = 1; i <= MaxClients; ++i) {
                if (IsClientInGame(i) && isSurvivor(i) && IsPlayerAlive(i) && !isIncapacitated(i) && !IsPinned(i)) {
                        new Float:target[3];
