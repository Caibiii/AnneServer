...
public Action Charger_OnPlayerRunCmd(int charger, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon) 
{
	...
	if(dist < 100.0 && ReadyAbility(charger))
	{
		buttons |= IN_ATTACK2;
		vel[0] = 0.0;
		vel[1] = 0.0;
	...
	else if(GetEntProp(charger, Prop_Send, "m_hasVisibleThreats") && g_hChargeProximity.IntValue + 10.0 < dist < 1000.0 && currentspeed > 175.0 && IsValidSurvivor(target)) 
	{
		if (flags & FL_ONGROUND)
		{
			...
			buttons |= IN_JUMP;
			buttons |= IN_DUCK;
			if (Do_Bhop(charger, buttons, vel_buffer))
			{
				return Plugin_Changed;
			}
		}
