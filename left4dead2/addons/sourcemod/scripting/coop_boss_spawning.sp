...
{
	InitGameData();
	g_cvShowBossPercRules = CreateConVar("l4d2_cbs_show_boss_percent_difficulties", "4", "Show a Bosses spawn percents for difficulties: 0 - Don't show for any difficulty | 1 - Easy | 2 - Normal and lower | 3 - Advanced and lower | 4 - Expert and lower");
...
public void Event_TankSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	switch(GetDifficultyIndex())
	{
		...
		case 3,4:
		{
			SetEntProp(client, Prop_Send, "m_iMaxHealth", 8000);
			SetEntProp(client, Prop_Send, "m_iHealth", 8000);
		}
	}
}
public Action L4D_OnGetScriptValueInt(const char[] key, int &retVal) 
{
	...
	if (strcmp(key, "TankLimit", false) == 0 || strcmp(key, "WitchLimit", false) == 0) 
    {
		retVal = 99;
		return Plugin_Handled;
	}
